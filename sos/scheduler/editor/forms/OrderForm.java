...
import sos.scheduler.editor.listeners.OrderListener;
...
public class OrderForm extends Composite implements IUnsaved, IUpdateLanguage {
	...
	public OrderForm(Composite parent, int style, DomParser dom,Element order,
	...
	private void createGroup1() {
		...
		.addModifyListener(new org.eclipse.swt.events.ModifyListener() {
			public void modifyText(org.eclipse.swt.events.ModifyEvent e) {
				if (event)listener.setCommandAttribute("job_chain",cJobchain.getText());
			}
		});
		...
		.addModifyListener(new org.eclipse.swt.events.ModifyListener() {
			public void modifyText(org.eclipse.swt.events.ModifyEvent e) {
				if (event)listener.setCommandAttribute("title",tTitle.getText());
			}
		});
		...
		.addModifyListener(new org.eclipse.swt.events.ModifyListener() {
			public void modifyText(org.eclipse.swt.events.ModifyEvent e) {
				if (event)listener.setCommandAttribute("priority",tPriority.getText());
			}
		});
		...
		.addModifyListener(new org.eclipse.swt.events.ModifyListener() {
			public void modifyText(org.eclipse.swt.events.ModifyEvent e) {
				if (event)listener.setCommandAttribute("state",tState.getText());
			}
		});
		...
		bReplace.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				String r="no";
				if (bReplace.getSelection()) r = "yes";
				if (event)listener.setCommandAttribute("replace",r);
			}
		});

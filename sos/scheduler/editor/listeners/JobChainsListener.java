...
public class JobChainsListener {
	...
	public JobChainsListener(DomParser dom, Element config) {
		_dom = dom;
		_config = config;
	...
	public void applyChain(String name, boolean ordersRecoverable,
			boolean visible) {
	...
	public void fillChain(Table table) {
		...
		if (_chain != null) {
			...
			while (it.hasNext()) {
				...
				if (!next.equals("")
						&& (state.equals("next") || !checkForState(next)))
					item.setBackground(2, Display.getCurrent().getSystemColor(
							SWT.COLOR_YELLOW));
				if (!error.equals("")
						&& (state.equals("error") || !checkForState(error)))
...
//		|| _node.getAttributeValue("error_state") != null;
	...
	public boolean isValidState(String state) {
		...
		for (int i = 0; i < _states.length; i++) {
			if (_states[i].equalsIgnoreCase(state)
					&& !_states[i].equals(getState()))
				return false;
		}
		return true;
	}

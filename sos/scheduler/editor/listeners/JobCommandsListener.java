...
public class JobCommandsListener {
	...
	public JobCommandsListener(DomParser dom, Element job, IUpdate update) {
		_dom = dom;
		_main = update;
		_job = job;
		if(_job != null)
	...
	private boolean haveCode(int code,Table table) {
		int count = table.getItemCount();
		 for (int i=0;i<count;i++) {
			  ...
			  if (actCode.indexOf(" " + String.valueOf(code)) >= 0) return true;
			  if (actCode.indexOf(String.valueOf(code) + " " ) >= 0) return true;
			  if (actCode.trim().equals(String.valueOf(code))) return true;
		  }
		return false;
	}
	...
	public void newCommands(Table table) {
		  ...
		  for (int i=0;i<count;i++) {
			  TableItem item = table.getItem(i);
			  if (item.getText().equals("success"))success=true;
			  if (item.getText().equals("error"))error=true;
		  }

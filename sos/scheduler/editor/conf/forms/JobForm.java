...
public class JobForm extends Composite implements IUnsaved, IUpdateLanguage {
	...
	private void createGroup2() {
		...
		//parForm.createParameter(sashForm);
	}
	
	/**
	 * This method initializes group2
	 */
	/*private void createGroup2_() {
		...
		final CTabItem parameterTabItem = new CTabItem(tabFolder, SWT.BORDER | SWT.CLOSE);
		...
		tParameter.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				butOpen.setEnabled(tableIncludeParams.getSelectionCount() > 0);
		...
		//Environment
		final CTabItem environmentTabItem = new CTabItem(tabFolder, SWT.CLOSE);
		...
		lblFilename.setLayoutData(new GridData(GridData.BEGINNING, GridData.FILL, false, false));
		...
		label4_2 = new Label(group_3, SWT.HORIZONTAL | SWT.SEPARATOR);
		label4_2.setLayoutData(new GridData(GridData.FILL, GridData.CENTER, false, false, 4, 1));
		label4_2.setText("Label");
		...
		tableIncludeParams.setLayoutData(new GridData(GridData.FILL, GridData.FILL, true, true, 3, 2));
	...
	*/
	...
	/*private void createParameterTabItem() {
			Element    params            = null;				
			final String filename = txtIncludeFilename.getText();
			final String node     = txtIncludeNode.getText();
			//System.out.println("open include Parameter from: " + filename);
			//final String filename = "C:/scheduler/config/param.xml";
			...
			try {
				...
				if(txtIncludeNode.getText() != null && txtIncludeNode.getText().length() > 0) {
					...
					System.out.println(Utils.getElementAsString(e));
					params = e;
				}else {
					params = doc.getRootElement();
				}
							
				final CTabItem includeParameterTabItem = new CTabItem(tabFolder, SWT.CLOSE);
				...
				txtIncludeParameter.addModifyListener(new org.eclipse.swt.events.ModifyListener() {
					public void modifyText(org.eclipse.swt.events.ModifyEvent e) {
						butoIncludeSave.setEnabled(!txtIncludeParameter.getText().trim().equals(""));
					}
				});
				...
				butImport.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(final SelectionEvent e) {
						//startWizzard(false);
						JobAssistentImportJobsForm importJobForms = new JobAssistentImportJobsForm(listener, tableIncludeParameter, Editor.JOB);					
							//importJobForms.setJobForm(this);
						importJobForms.showAllImportJobs();
					}
				});
				...
				butoIncludeSave.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
					public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
						...
						//IOUtils.saveXML(parentDoc, group_1.getText());
					}
				});

...
/**
 * Style Class containing the default style for the GUI components used in the
 * DaySelector as well as accessors to the component templates. This class is
 * meant to be extended by users longing to set their own definitions. The
 ...
 * the style class. For additional conveniance a DaySelectorStyleAdapter is
 * provided.
 * 
 * @author Will Roethel
...
public class DaySelectorStyle extends DateChooserStyle {
    ...
    protected static String[] weekdayNameDefault = { "S", "M", "T", "W", "T", "F", "S" };
    ...
    protected final void setDefaultStyle(Control control, int style) {
        ...
        switch (style) {
            ...
            case BLANKLABEL:
                ...
                label.setForeground(Display.getCurrent().getSystemColor(SWT.COLOR_TITLE_INACTIVE_FOREGROUND));
                break;
            ...
            case SELECTEDLABEL:
                label.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW));
                label.setForeground(Display.getCurrent().getSystemColor(SWT.COLOR_BLACK));
                break;
     ...
     * length 7 otherwise setWeekdayNames will throw an exception.
     * 
    ...
    public final void setWeekdayNames(String[] weekdays) throws Exception {
        if (weekdays.length != 7) {
            throw new Exception("Expected String[] with length 7. Found length " + weekdays.length

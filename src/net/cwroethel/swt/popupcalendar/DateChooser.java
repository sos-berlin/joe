...
/**
 * <p>
 * Panel containing the actual calendar. Can also be used independent of
 * JPopupCalendar just by creating a DateChooser and calling the paint() method.
 * </p>
 * <p>
 * DateChooser is made up of three parts:
 * <ul>
 ...
 * PopupCalendar.SHOWALL. Rearranging the components as for the Swing version of
 * DateChooser is currently not possible. DateChooser can be used as a
 * standalone widget:
 * 
 * <pre>
 *      ...
...
public class DateChooser extends DateChooserPanel {
    ...
    /**
     * Default constructor. Style options are SHOWMONTH, SHOWTODAY OR SHOWALL
     * and all options supported by Composite. The default locale will be used.
     * 
     ...
     * calling pack for these to be visible. The components are only created
     * once and reused by re-labeling and rearranging listeners. This makes
     * updating about 6-7 times faster.
     */
    public void paint() {
    ...
    /**
     * Initializes the calendar panel and draws all components. This is only
     * needed the first time the calendar is created. All successive uses will
     * update existing calendar components.
     * 
     * @throws Exception
     */
    private void initCalendarPane() throws Exception {
        ...
        if (showMonthSelector) {
            ...
            monthSelector.setMonth(getMonthSelectorStyle().getMonthName(currentDate.get(java.util.Calendar.MONTH)));
            monthSelector.setYear(currentDate.get(java.util.Calendar.YEAR));

            // add the listener
            monthSelector.addSelectionListener(new SelectionAdapter() {
                public void widgetSelected(SelectionEvent e) {
                    MonthSelector ms = (MonthSelector) e.widget;
                    ...
                    if (e.detail == DateChooserAction.monthSelected) {
                        currentDate.set(java.util.Calendar.MONTH, ms.getMonth());
                        paint();
                    }
                    if (e.detail == DateChooserAction.yearSelected) {
                        currentDate.set(java.util.Calendar.YEAR, ms.getYear());
                        paint();
                    }
                }
            });
        ...
        // day selector is put into an additional Composite to make the layout
        // look
        ...
        dayPaneLayout.marginHeight = 4;
        dayPaneLayout.marginWidth = 0;
        ...
        daySelector.setLayoutData(data);
    ...
    public void updateCalendar() {
        setVisible(false);
        if (monthSelector != null && monthSelector.doUpdate()) {
            monthSelector.setMonth(getMonthSelectorStyle().getMonthName(currentDate.get(java.util.Calendar.MONTH)));
            monthSelector.setYear(currentDate.get(java.util.Calendar.YEAR));
        }

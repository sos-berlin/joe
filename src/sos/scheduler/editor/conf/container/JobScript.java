...
public class  JobScript extends FormBaseClass {
    ...
    public JobScript(Composite pParentComposite, JobListener pobjJobDataProvider,JobScript that) {
    ...
    private void getValues(JobScript that){
        if (that == null){
            return;
        }
        
        this.tSource.setText(that.tSource.getText());        
     }
        ...
        cboPrefunction.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(final SelectionEvent e) {
                if (cboPrefunction.getText().length() > 0) {
                    String lan = "function_" + ((type == Editor.MONITOR) ? "monitor" : "job") + "_" + objJobDataProvider.getLanguage(objJobDataProvider.getLanguage()) + "_";

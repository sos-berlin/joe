...
public class ScriptFormPreProcessing extends ScriptForm implements IUpdateLanguage {
	...
	public ScriptFormPreProcessing(final Composite parent, final int style, final SchedulerDom dom, final Element job, final ISchedulerUpdate main) {
				...
				if (favorites != null && favorites.get(preProcessingHeader.getCboFavorite().getText()) != null
					...
					if (objDataProvider.isJava()) {
						...
						for (String element : split) {
							objDataProvider.addInclude(element);
						}
					}
					fillForm();
				}

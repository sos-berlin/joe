...
public class JobStartWhenDirectoryChanged extends FormBaseClass {
	...
	private void createGroup() {
		...
		tDirectory.setDataProvider(objDataProvider);
	...
	private void initDirectory(boolean enabled) {
		...
		if (enabled) {
			tDirectory.setText(objDataProvider.getDirectory());
			tRegex.setText(objDataProvider.getRegex());
		}
		else {
			tDirectory.setText("");
			tRegex.setText("");
	...
	private void applyDirectory() {
		...
		if (Utils.isRegExpressions(strRegExp)) {
			objDataProvider.applyDirectory(tDirectory.getText(), strRegExp);
			objDataProvider.fillDirectories(tDirectories);
			initDirectory(false);
	...
	private void createTable3() {
		...
		tDirectories.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (tDirectories.getSelectionCount() > 0) {
					objDataProvider.selectDirectory(tDirectories.getSelectionIndex());
					initDirectory(true);
					tDirectory.setFocus();
				}
				else

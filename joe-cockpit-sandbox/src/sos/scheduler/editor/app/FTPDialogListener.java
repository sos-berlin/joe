...
public class FTPDialogListener {
			...
			if (ftpClient instanceof SOSFTP) {
				if (directory.startsWith("./")) {
					if (workingDirectory.length() >= 0 && workingDirectory.equalsIgnoreCase(directory)) {
						...
						if (workingDirectory.length() >= directory.length() && workingDirectory.startsWith(directory)) {
							String curWorkingDirectory = workingDirectory.substring(directory.length());
							curWorkingDirectory = curWorkingDirectory.endsWith("/") ? curWorkingDirectory.substring(0, curWorkingDirectory.length())
									: curWorkingDirectory;
							...
							if (directory.startsWith(workingDirectory) || workingDirectory.length() == 0) {
	...
	public void connect(final String profile) {
		...
		try {
			...
			authenticationMethod = sosString.parseToString(prop.get("auth_method")).length() > 0 ? sosString.parseToString(prop.get("auth_method"))
					: "publickey";
					...
					if (protocol.equalsIgnoreCase("ftps")) {
						try {
							...
							if (!isLoggedIn || sosftps.getReplyCode() > ERROR_CODE) {
								throw new Exception("..ftp server reply [login failed] [user=" + user + "], [account=" + account + "]: "
			...
			if (logtext != null)
				logtext.append("..error in ftp server init with [host=" + host + "], [port=" + port + "], cause: " + getErrorMessage(ex) + "\n");
			if (tryAgain) {
				if (logtext != null)
				...
				while (c.hasNext()) {
					...
					if (changes.get(remFile) != null && changes.get(remFile).equals("delete")) {
						...
						if (remFile.startsWith("job_chain"))
							...
							if (remFile.startsWith("job"))
								prefix = "job";
							else
								if (remFile.startsWith("order"))
									prefix = "order";
								else
									if (remFile.startsWith("add_order"))
										prefix = "add_order";
									else
										if (remFile.startsWith("process_class"))
											prefix = "process_class";
										else
											if (remFile.startsWith("lock"))
												prefix = "lock";
											else
												if (remFile.startsWith("schedule"))
													prefix = "schedule";

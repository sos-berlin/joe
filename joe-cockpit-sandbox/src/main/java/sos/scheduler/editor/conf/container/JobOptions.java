...
public class JobOptions extends FormBaseClass<JobListener> {
	...
	public JobOptions(final Composite pParentComposite, final JobListener pobjJobDataProvider) {
	...
	public void createGroup() {
		...
		bOrderYes.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				JSObjJob objJob = objJobDataProvider.getJSJob();
				JSObjRunTime objRunTime = objJob.getRunTimeObj();
				if (objJobDataProvider.getOrder() && objJob != null && objRunTime != null) {
					if (objRunTime.hasSingleStart() == true || objRunTime.isLetRun() == true || objRunTime.isOnce() == true) {
						...
						// ToDo should be implemented in JSobjRunTime class as part of the save operation
						if (_deleteRuntimeAttribute) {
							objRunTime.setSingleStart(false);
							objRunTime.setLetRun(false);
							objRunTime.setOnce(false);
						}
					}
				}
			}
		...
		logLevel.setText(objJobDataProvider.getLogLevel());

		logLevel.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				objJobDataProvider.setLogLevel(logLevel.getText());
			}
		});
		logLevel.setItems(JSObjJob.ValidLogLevels);
	...
	private void createMainOptionsGroup() {
		...
		cSignals.setItems(new String[] { "SIGHUP", "SIGINT", "SIGQUIT", "SIGILL", "SIGTRAP", "SIGABRT", "SIGIOT", "SIGBUS", "SIGFPE", "SIGKILL", "SIGUSR1",
				"SIGSEGV", "SIGUSR2", "SIGPIPE", "SIGALRM", "SIGTERM", "SIGSTKFLT", "SIGCHLD", "SIGCONT", "SIGSTOP", "SIGTSTP", "SIGTTIN", "SIGTTOU", "SIGURG",

...
public class HttpAuthenticationForm extends SOSJOEMessageCodes implements IUnsaved, IUpdateLanguage {
	...
	private final HttpAuthenticationListener listener;
	...
	public HttpAuthenticationForm(final Composite parent, final int style, final SchedulerDom dom, final Element config) {
...
//		butRemoveHttpUser.setToolTipText(Messages.getTooltip("http_authentication.remove_button"));
	...
	private void encrypt() {
		try {
			...
			try {
//    			new ErrorLog("error in " + SOSClassUtil.getMethodName() +  txtPassword.getText() + " could not encrypt.", ex);

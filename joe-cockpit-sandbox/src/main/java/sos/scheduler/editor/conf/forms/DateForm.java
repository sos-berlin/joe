...
public class DateForm extends SOSJOEMessageCodes implements IUpdateLanguage {
	...
	public DateForm(final Composite parent, final int style, final int type) {
		super(parent, style);
		this.type = type;
		initialize();
		setToolTipText();
	...
	public DateForm(final Composite parent, final int style, final int type, final SchedulerDom dom, final Element element, final ISchedulerUpdate main) {
		this(parent, style, type);
		setObjects(dom, element, main);
		setNow();
		// this.gDates.setEnabled(Utils.isElementEnabled("job", dom, element));
		gDates.setEnabled(Utils.isElementEnabled("job", dom, element) && !Utils.hasSchedulesElement(dom, element));
		bAddDay.setFocus();
	}

	public void setObjects(final SchedulerDom dom, final Element element, final ISchedulerUpdate main) {

...
public class JobDelayAfterError extends FormBaseClass<JobListener> {
	...
	public JobDelayAfterError(final Composite pParentComposite, final JobListener pobjDataProvider) {
			...
			if (maxErrorDelay > 0 && maxErrorDelay < Utils.str2int(sErrorCount.getText()) || maxAktErrorDelay > Utils.str2int(sErrorCount.getText())
				...
				if (maximum > 1) {
					...
					if (sErrorCount.getText().equals("")) {
						...
						objJobDataProvider.fillErrorDelayTable(tErrorDelay);

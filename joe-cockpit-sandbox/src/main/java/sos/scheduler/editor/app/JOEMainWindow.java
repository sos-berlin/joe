...
public class JOEMainWindow {
	...
	private void createContainer(final Composite objParent) {
	...
	public class ClearStatusLine extends TimerTask {
		@Override
		public void run() {
			StatusLine.setText("");
		}
	 ...
	 * @wbp.parser.entryPoint
	 */
	public void createSShell(final Composite containerParent, final Composite objParent) {
		...
		pOpenWebDav.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(final org.eclipse.swt.events.SelectionEvent e) {
				try {
					...
					try {
						new ErrorLog("error in " + SOSClassUtil.getMethodName() + " ; could not open file on Webdav Server",
								ex);
					}
					catch (Exception ee) {
					}
...
//		pExit.setAccelerator(SWT.CTRL | 'E');
			...
			public void widgetSelected(final org.eclipse.swt.events.SelectionEvent e) {
				try {
					...
					sShell.close();
						...
						new ErrorLog("error: " + SOSClassUtil.getMethodName(), es);
				}
			}
		...
		sShell.addShellListener(new ShellAdapter() {
			...
			@Override
			public void shellActivated(final org.eclipse.swt.events.ShellEvent e) {
				setSaveStatus();
			}
		});
...
*/
	...
	public static int message(final String message, final int style) {
	...
	// }
	// }
	public static int message(final Shell shell, String pstrMessage, final int style) {
...
//			changes = (java.util.HashMap) currdom.getListOfChangedObjects().clone();
		...
		butShowAsSML.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				try {
					...
					try {
						new ErrorLog("error in " + SOSClassUtil.getMethodName() + " cause: " + ex.toString(), ex);
					}
					catch (Exception ee) {
						// tu nichts
					}
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				if (container.getCurrentEditor() instanceof SchedulerForm) {
					...
					if (container.getCurrentEditor() instanceof ActionsForm) {
						...
						ActionsDom currdom = form.getDom();
						Utils.reset(currdom.getRoot(), form, currdom);
					}
					else
	...
	public static void saveFTP(final java.util.HashMap changes) {
		try {
	...
	private void saveWebDav(final java.util.HashMap changes) {
	...
	private boolean existLibraries() {
		...
		try {
			...
			try {
				new ErrorLog("error in " + SOSClassUtil.getMethodName() + " cause: " + msg, e);
			} catch(Exception ee) {
	...
	private void startWizard() {
		try {
			...
			try {
				new ErrorLog("error in " + SOSClassUtil.getMethodName() + " ; could not start wizard.", ex);
			}
			catch (Exception ee) {
				// tu nichts
			}
...
//										newFFiles.add(fFile);
			...
			try {
				new ErrorLog(Messages.getString("exception.raised", SOSClassUtil.getMethodName()), e);
			}
			catch (Exception ee) {
				// tu nichts
			}
	 ...
	 * @return
	 */
	private static String getXPathString(final File hFfile, final boolean onlyParentPath) {
		String aName = EMPTY;
		String eName = EMPTY;

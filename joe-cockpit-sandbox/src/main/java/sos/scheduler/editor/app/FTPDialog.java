		...
		try {
			...
			if (f.exists() && f.list().length > 0) {
				...
				for (String element : list) {
					if (element != null
							&& (element.endsWith(".job.xml") || element.endsWith(".job_chain.xml") || element.endsWith(".order.xml")
									|| element.endsWith(".lock.xml") || element.endsWith(".process_class.xml") || element.endsWith(".config.xml") || element.endsWith(".schedule.xml"))) {
						l.add(element);
					}
				}
			}

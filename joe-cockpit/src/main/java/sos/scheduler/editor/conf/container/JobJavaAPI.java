...
public class JobJavaAPI extends FormBaseClass {
	...
	private void getValues(JobJavaAPI that) {
		...
		if (that.tbxClassName != null && this.tbxClassName != null) {
	...
	private void createGroup() {
		...
		if (objJobDataProvider.isJava()) {
			Label lblClassNameLabel = SOSJOEMessageCodes.JOE_L_JobJavaAPI_Classname
					.control(new Label(gScript, SWT.NONE));
		...
		final Label java_optionsLabel = SOSJOEMessageCodes.JOE_L_JobJavaAPI_Options
				.control(new Label(gScript, SWT.NONE));
		...
		if (objJobDataProvider.isJava()) {
			...
			txtJavaOptions.setLayoutData(gd_tClasspath);
		}

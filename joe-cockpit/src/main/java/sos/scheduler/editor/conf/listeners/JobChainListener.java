...
public class JobChainListener extends JOEListener implements IProcessClassDataProvider {
    ...
    public void fillChain(final Table table) {
        ...
        if (_chain != null) {
            ...
            while (it.hasNext()) {
                state = "";
                Element node = (Element) it.next();
                if (node.getName().equals("job_chain_node") || node.getName().equals("job_chain_node.end")
                        || node.getName().equals("file_order_sink")) {
                    state = Utils.getAttributeValue("state", node);
                    if (node.getName().equals("job_chain_node")) {
                        if (Utils.getAttributeValue("job", node) == "") {
                            nodetype = "Endnode";
                            node.removeAttribute("next_state");// kann eventuell
                                                               // bei reorder
                                                               // changeup
                                                               // entstanden
                                                               // sein
    ...
    private JobchainReturnCodeAddOrderElement addOrderParameters(org.w3c.dom.Element addOrderParams,
        ...
        // List<Element> listOfParams =
        // addOrderParams.getChildren("param",addOrder.getNamespace());
    ...
    public void applyNode(final boolean isJobchainNode, final String state, final String job, final String delay, final String next,
        ...
        try {
            ...
            if (_node != null) {// Wenn der Knotentyp geändert wird, alten
                ...
                if (oldState != null && state != null && !oldState.equals(state)) {
                    // state hat sicg geändert. ggf die Details state auch
                    // ändern
        ...
        try {
            ...
            if (reorder) {
                ...
                if (Utils.getAttributeValue("job", _node).length() == 0 || _node != null && _node.getName().equals("job_chain_node.end")) {
                    // sos.scheduler.editor.app.MainWindow.message("Only Job Chain Node could be Reorder",
            ...
            for (int i = 0; i < _chain.getContentSize(); i++) {
                if (l.get(i) instanceof Element) {
                    ...
                    // String elemState = Utils.getAttributeValue("state",
                    // elem_);
                    // String nodeState = Utils.getAttributeValue("state",
                    // _node);
                    if (up) {
                        // up
                        // if(elemState.equals(nodeState)) {
                        if (elem_.equals(_node)) {
                            // System.out.println("up " + elemState + "=" +
                            // nodeState + "    elem_.equals(_node) "
                            // +elem_.equals(_node) );
                            break;
                        } else {
                            cIndex = i;
                        ...
                        // down
                        if (elem_.equals(_node)) {
                            // System.out.println("down " + elemState + "=" +
                            // nodeState + "    elem_.equals(_node) "
                            // +elem_.equals(_node) );
                            found = true;
                        } else if (found) {
    ...
    /*
     * public void applyReturnCodes(){ Element onReturnCodes =
     * getOnReturnCodes(); if (_node != null) { if (onReturnCodes != null){
     * Element e = _node.getChild("on_return_codes"); if (e != null){
     * e.detach(); } _node.addContent(onReturnCodes); _dom.setChanged(true);
     * _dom.setChangedForDirectory("job_chain", Utils.getAttributeValue("name",
     * _chain), SchedulerDom.MODIFY); } } }
     */

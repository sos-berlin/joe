...
public class JobChainNestedListener {
    ...
    public void fillChain(Table table) {
        ...
        if (_chain != null) {
            setStates();
            // Iterator it =
            // _chain.getChildren("job_chain_node.job_chain").iterator();
            ...
            while (it.hasNext()) {
                ...
                if (node.getName().equals("job_chain_node.job_chain")) {
                    ...
                    state = Utils.getAttributeValue("state", node);
                    TableItem item = new TableItem(table, SWT.NONE);
                    // item.setText(new String[] { state, nodetype, action,
                    // next, error, onError });
                    item.setText(new String[] { state, "Endnode", "", "", "", "" });
                }
                /*
                 * Iterator itEndNode =
                 * _chain.getChildren("job_chain_node.end").iterator(); while
                 * (itEndNode.hasNext()) { Element node = (Element)
                 * itEndNode.next(); state = Utils.getAttributeValue("state",
                 * node); TableItem item = new TableItem(table, SWT.NONE);
                 * //item.setText(new String[] { state, nodetype, action, next,
                 * error, onError }); item.setText(new String[] { state,
                 * "Endnode", "", "", "", "" }); }
                 */
            }
        }
    ...
    /*
     * public void selectFileOrderSource(Table tableSources) { if (tableSources
     * == null){ _source = null; }else { int index =
    ...
    public void applyNode(boolean isJobchainNode, String state, String job, String next, String error, boolean isFullnode) {
        Element node = null;
        if (_node != null) {
            // System.out.println("node != null, old state=" +
            // Utils.getAttributeValue("state", _node) + ", new state=" +
            // state);
    ...
    public void changeUp(Table table, boolean up, boolean isJobchainNode, String state, String job, String delay, String next, String error, int index,
        ...
        try {
            Element node = null;
            if (reorder) {
                if (Utils.getAttributeValue("job_chain", _node).length() == 0 || (_node != null && _node.getName().equals("job_chain_node.end"))) {
            ...
            if (reorder) {
                ...
                // for( java.util.Iterator it = elements.iterator();
                // it.hasNext(); ) {

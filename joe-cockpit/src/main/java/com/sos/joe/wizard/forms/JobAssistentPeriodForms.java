...
public class JobAssistentPeriodForms extends JobWizardBaseForm {
    ...
    public void showPeriodeForms() {
        try {
            ...
            shell.setText(SOSJOEMessageCodes.JOE_M_JobAssistent_RunTimePeriods.label());
            {
                ...
                tabFolder.setLayoutData(gridData);
                {
                    ...
                    everyDayTabItem.setText(SOSJOEMessageCodes.JOE_M_JobAssistent_EveryDay.label());
                    {
                        ...
                        // createPeriodForm(SOSJOEMessageCodes.JOE_JobAssistent_EveryDayTabItem.label(),
                        // group, everyDayTabItem);
                    }
                }
                {
            ...
            shell.open();
            // Button butHelp = Utils.createHelpButton(shell,
            // "assistent.period", shell);
            ...
            // System.err.println("..error in JobAssistentPeriodFormss.showPeriodeForms() "
            // + e.getMessage());
    ...
    private Element getMonthElement() {
        ...
        if (days != null && days.length > 0) {// gleich 1 bedeutet, das day
                                              // Element gerade generiert wurde
        ...
        if (day == null) {
            ...
            day = monthListener.getDayElements()[monthListener.getDayElements().length - 1];// letzte
                                                                                            // Element
            periodsListenerMonthDay = new PeriodsListener(dom, day);
        }
        return day;
    ...
    private Element getDayElement() {
        ...
        if (day == null) {
            ...
            day = weekDayListener.getDayElements()[weekDayListener.getDayElements().length - 1];// letzte
                                                                                                // Element
            periodsListenerWeekDay = new PeriodsListener(dom, day);
        }
        return day;
    ...
    private void apply() {
        ...
        if (tabFolder.getSelection()[0].getText().equals(JobAssistentPeriodForms.EVERY_DAY)) {
            ...
            periodForm.setPeriod(period);
            periodForm.setEnabled(false);
            // list.add(JobAssistentPeriodForms.EVERY_DAY + "at " +
            // periodForm.getListener().getBegin() + "- " +
            // periodForm.getListener().getEnd());
            ...
            if (comboWeekDay.getSelectionIndex() == -1 || comboWeekDay.getText().length() == 0) {
                // ErrorLog.message(shell,
                // sos.scheduler.editor.app.Messages.getString("assistent.period.date"),
            ...
            Element period = periodFormWeekDay.getPeriod();// .getDayElements()[weekDayListener.getDayElements().length
                                                           // - 1];//letzte
                                                           // Element
            periodFormWeekDay.setEnabled(false);
            periodsListenerWeekDay.applyPeriod(period);
            update.updateDays(DaysListener.WEEKDAYS);
            // list.add(JobAssistentPeriodForms.WEEK_DAY +
            // comboWeekDay.getText() + " at " +
            // periodFormWeekDay.getListener().getBegin() + "-" +
            ...
            // list.add(JobAssistentPeriodForms.MONTH_DAY + comboMonth.getText()
            // + " at " + periodFormMonthDay.getListener().getBegin() + "-" +
            ...
            // list.add(JobAssistentPeriodForms.SPECIFIC_DAY +
            // txtSpeDay.getISODate() + " at " +
            // periodFormSpecificDay.getListener().getBegin() + "-" +
            // periodFormSpecificDay.getListener().getEnd());
    ...
    private void fillList() {
        ...
        for (int i = 0; speDays != null && i < speDays.size(); i++) {
            ...
            for (int j = 0; periods != null && j < periods.size(); j++) {
                ...
                if (p.getSingle() == null || p.getSingle().trim().length() == 0) {
                    // list.add(JobAssistentPeriodForms.SPECIFIC_DAY +
                    // Utils.asStr(da[2]) + "-" + Utils.asStr(da[1]) + "-" +
                    // Utils.asStr(da[0]) + " at " + p.getSingle());
        ...
        for (int i = 0; weekDays != null && i < weekDays.length; i++) {
            ...
            for (int j = 0; periods != null && j < periods.size(); j++) {
                ...
                p.setPeriod(period);
                if (p.getSingle() == null || p.getSingle().trim().length() == 0) {
                    // list.add(JobAssistentPeriodForms.WEEK_DAY + sWeek +
                    // " at " + p.getBegin() + "-" + p.getEnd());
        ...
        for (int i = 0; monthDays != null && i < monthDays.length; i++) {
            ...
            for (int j = 0; periods != null && j < periods.size(); j++) {
                ...
                p.setPeriod(period);
                if (p.getSingle() == null || p.getSingle().trim().length() == 0) {
                    // list.add(JobAssistentPeriodForms.MONTH_DAY + sMonth +
                    // " at " + p.getBegin() + "-" + p.getEnd());
    ...
    private void deleteEveryDay(String selectedStr) {
        ...
        for (int i = 0; everyDay != null && i < everyDay.size(); i++) {
            ...
            PeriodListener p = new PeriodListener(dom);
            p.setPeriod(period);
            // if (selectedStr.equals(JobAssistentPeriodForms.EVERY_DAY + "at "
            // + p.getSingle())) {
    ...
    private void deleteSpeDay(String selectedStr) {
        ...
        for (int i = 0; speDays != null && i < speDays.size(); i++) {
            ...
            if (periods == null || periods.size() <= 1) {
                // es existiert nur maximal eine Periode -> der Element kann
                // gelöscht werden
                ...
                for (int j = 0; periods != null && j < periods.size(); j++) {
                    ...
                    p.setPeriod(period);
                    // if
                    // (selectedStr.equals(JobAssistentPeriodForms.SPECIFIC_DAY
                    // + Utils.asStr(da[2]) + "-" + Utils.asStr(da[1]) + "-" +
                    // Utils.asStr(da[0]) + " at " + p.getSingle())) {
        ...
        for (int i = 0; weekDays != null && i < weekDays.length; i++) {
            ...
            if (selectedStr.indexOf(sWeek) > -1) {
                ...
                if (periods == null || periods.size() <= 1) {
                    ...
                    // Der Wochentag hat mehr als eine Periode -> lösche den mit
                    // der gleichen Zeit
                    for (int j = 0; periods != null && j < periods.size(); j++) {
                        Element period = (Element) periods.get(j);
                        PeriodListener p = new PeriodListener(dom);
                        p.setPeriod(period);
                        // String time =
                        // selectedStr.substring(selectedStr.indexOf("at ") +
                        // 3);
        ...
        for (int i = 0; monthDays != null && i < monthDays.length; i++) {
            ...
            if (selectedStr.indexOf(sMonth) > -1) {
                ...
                if (periods == null || periods.size() <= 1) {
                    ...
                    // Der Monat hat mehr als eine Periode -> lösche den mit der
                    // gleichen Zeit
                    for (int j = 0; periods != null && j < periods.size(); j++) {
                        Element period = (Element) periods.get(j);
                        PeriodListener p = new PeriodListener(dom);
    ...
    private void close() {
        // int cont = ErrorLog.message(shell,

...
public class JobAssistentDelayAfterErrorForm extends JobWizardBaseForm {
    ...
    public void showDelayAfterErrorForm() {
        ...
        // butBack.setText("Back");
        {
            ...
            // "assistent.delay_after_error", shellSetBack);
    ...
    private boolean check() {
        String sTime = (txtHour.getText() != null && !txtHour.getText().equals("00") ? txtHour.getText() : "").concat(txtMin.getText() != null
                && !txtMin.getText().equals("00") ? txtMin.getText() : "").concat(txtSecound.getText() != null && !txtSecound.getText().equals("00") ? txtSecound.getText()
                : "");
        String errorCount = (txtErrorCount.getText() != null ? txtErrorCount.getText() : "");
        if (sTime.length() == 0 && errorCount.length() > 0) {
            // ErrorLog.message(shellSetBack,
            // Messages.getString("assistent.delay_after_error.time.missing"),
            // SWT.OK );
        ...
        if (sTime.length() > 0 && errorCount.length() == 0) {
            // ErrorLog.message(shellSetBack,
            // Messages.getString("assistent.delay_after_error.error_count.missing"),
            // SWT.OK );

...
public class JobAssistentRunTimeForms extends JobWizardBaseForm {
    ...
    public void showRunTimeForms() {
        try {
            ...
            }
            {
                ...
                // Utils.createHelpButton(composite, "assistent.run_time",
                // runTimeSingleShell);
            ...
            // System.err.println("..error in JobAssistentRuntimeForms.showRunTimeForms() "
            // + e.getMessage());
    ...
    private void addPeriod() {
        if (optEveryDay.getSelection()) {
            if (txtDayAtHour.getText().concat(txtDayAtMinutes.getText()).concat(txtDayAtSecound.getText()).trim().length() > 0) {
                // String str = EVERY_DAY + " at " + Utils.getTime(23,
                // txtDayAtHour.getText(), txtDayAtMinutes.getText(),
                // txtDayAtSecound.getText(), false);
        ...
        if (optSpecificDay.getSelection()) {
            if (txtSpeDay.getISODate() != null && txtSpeDay.getISODate().trim().length() > 0) {
                ...
                // list.add(SPECIFIC_DAY + txtSpeDay.getISODate() + " at " +
                // Utils.getTime(23, txtSpeDayHour.getText(),
                // txtSpeDayAtMinutes.getText(), txtSpeDayAtSecound.getText(),
                // false));
        ...
        if (optEveryWeeksdays.getSelection()) {
            if (comboEveryWeekdays.getText() != null && comboEveryWeekdays.getText().trim().length() > 0) {
                savePeriod(WEEK_DAY);
                // list.add(WEEK_DAY + comboEveryWeekdays.getText() + " at " +
                // Utils.getTime(23, txtWeekAtHour.getText(),
                // txtWeekAtMinutes.getText(), txtWeekAtSecound.getText(),
                // false));
        ...
        if (optEveryMonths.getSelection()) {
            if (comboMonth.getText() != null && comboMonth.getText().trim().length() > 0) {
                ...
                // list.add(MONTH_DAY + comboMonth.getText() + " at " +
                // Utils.getTime(23, txtMonthAtHour.getText(),
                // txtMonthAtMinutes.getText(), txtMonthAtSecound.getText(),
                // false));
    ...
    private boolean periodExist(String str) {
        for (int i = 0; i < list.getItemCount(); i++) {
            ...
            if (currStr.equalsIgnoreCase(str)) {
                // ErrorLog.message(runTimeSingleShell,
                // sos.scheduler.editor.app.Messages.getString("assistent.run_time.period_exist"),
    ...
    private void fillList() {
        ...
        for (int i = 0; speDays != null && i < speDays.size(); i++) {
            ...
            for (int j = 0; periods != null && j < periods.size(); j++) {
                ...
                p.setPeriod(period);
                if (p.getBegin() == null || p.getBegin().trim().length() == 0) {
                    // list.add(SPECIFIC_DAY + Utils.asStr(da[2]) + "-" +
                    // Utils.asStr(da[1]) + "-" + Utils.asStr(da[0]) + " at " +
                    // p.getSingle() );
        ...
        for (int i = 0; speDays != null && i < speDays.size(); i++) {
            ...
            if (periods == null || periods.size() <= 1) {
                ...
                for (int j = 0; periods != null && j < periods.size(); j++) {
                    ...
                    PeriodListener p = new PeriodListener(dom);
                    p.setPeriod(period);
                    // if(selectedStr.equals(SPECIFIC_DAY + Utils.asStr(da[2]) +
                    // "-" + Utils.asStr(da[1]) + "-" + Utils.asStr(da[0]) +
                    // " at " + p.getSingle() )){
    ...
    private boolean checkTime(String time, String which) {
        ...
        if (!Utils.isOnlyDigits(time)) {
            // ErrorLog.message(runTimeSingleShell,
            // sos.scheduler.editor.app.Messages.getString("assistent.no_numeric"),
            // SWT.ICON_WARNING | SWT.OK |SWT.CANCEL );
            ...
            if (which.equals("hour")) {
                ...
                if (itime < 0 || itime > 24) {
                    // ErrorLog.message(runTimeSingleShell,
                    // sos.scheduler.editor.app.Messages.getString("assistent.no_time"),
                    // SWT.ICON_WARNING | SWT.OK |SWT.CANCEL );
    ...
    private void close() {
        // int cont = ErrorLog.message(runTimeSingleShell,

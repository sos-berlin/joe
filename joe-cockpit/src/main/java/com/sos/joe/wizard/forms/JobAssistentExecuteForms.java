...
public class JobAssistentExecuteForms {
    ...
    public void showExecuteForm() {
        ...
        shell.open();
        final Composite composite1 = SOSJOEMessageCodes.JOE_Composite1.Control(new Composite(shell, SWT.NONE));
        final GridLayout gridLayout_2_1 = new GridLayout();
        gridLayout_2_1.marginWidth = 0;
        gridLayout_2_1.horizontalSpacing = 0;
        composite1.setLayout(gridLayout_2_1);
        butCancel = SOSJOEMessageCodes.JOE_B_JobAssistent_Cancel.Control(new Button(composite1, SWT.NONE));
    ...
    private void refreshJob() {
        if (butProcess.getSelection()) {
            ...
            if (comLanguage.getSelectionIndex() > 1 && txtInclude.getText() != null && !txtInclude.getText().trim().isEmpty() 
                    && (scriptlistener.getIncludes().length == 0 || !scriptlistener.getIncludes()[scriptlistener.getIncludes().length - 1].equals(txtInclude.getText()))) {

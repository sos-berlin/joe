...
public class ParameterForm extends SOSJOEMessageCodes implements IUnsaved  {
	...
	private void initialize() {
		...
		try {
			isRemoteConnection =  false ; // sosString.parseToString(MainWindow.getContainer().getCurrentTab().getData("ftp_title")).length() > 0;
		}
		catch (Exception e) {
		}
	...
	private void createIncludes() {
		...
		if (type == JOEConstants.JOB || type == JOEConstants.COMMANDS || type == JOEConstants.JOB_COMMANDS) {
			...
			@SuppressWarnings("unused") final Label lblNode_ = JOE_L_ActionsParameterForm_File.Control(new Label(group_3, SWT.NONE));
		}
		...
		@SuppressWarnings("unused") final Label lblNode = JOE_L_ActionsParameterForm_Node.Control(new Label(group_3, SWT.NONE));

...
public class FTPDialogListener {
    ...
    public FTPDialogListener(File configfile) throws Exception {
        ...
        try {
            ...
            hasError = true;
            FTPProfile.log("error in " + sos.util.SOSClassUtil.getMethodName() + "could not read Profiles from " + configFile + ", cause: " + e.toString(), 
                    SOSLogger.WARN);
    ...
    public void saveProfile(boolean savePassword) throws Exception {
        try {
            ...
            try {
                if (savePassword && !profile.getPassword().isEmpty()) {
                    String encrypt = "";

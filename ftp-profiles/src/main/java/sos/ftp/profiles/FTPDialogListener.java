...
public class FTPDialogListener {
     ...
     * Unterhalb dieser Sektionen werden die Einstellungen/Zugangsdaten
     * gespeichert. */
    ...
    /** Falls es nicht erwünscht ist, den Passwort zuspeichern, dann hat man auch
     * die Möglichkeit dieser während der Laufzeit anzugeben. */
    ...
    /** Liefert ein HaspMap Objekt. Dieser beinhaltet als name=profilname und
     * value=entsprechende FTPProfile Objekt
     * 
     * @return */
    ...
    /** Liest den Inhalt der Datei file und gibt den Inhalt als Array von Byte
     * zurück.
     * 
     * @param file
     * @return
        ...
        try {
            ...
            try {
                if (savePassword && profile.getPassword().length() > 0) {
                    ...
                    String encrypt = "";
                    if (profile.getPassword().endsWith("=")) {// ist bereits
                                                              // encrypted
            ...
            hasError = true;
            FTPProfile.log("error in " + sos.util.SOSClassUtil.getMethodName() + "could not save configurations File: " + configFile + ", cause: "
                    + e.toString(), SOSLogger.WARN);

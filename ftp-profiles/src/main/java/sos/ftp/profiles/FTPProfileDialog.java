...
public class FTPProfileDialog {
	...
	public void showModal(Combo combo_) {
		try {
			...
			new Label(groupSftp, SWT.NONE);
			butAuthPublicKey = new Button(groupSftp, SWT.RADIO);
			...
			butAuthPublicKey.setLayoutData(gridData_pk);
			butAuthPublicKey.setText("Public Key");
			butAuthPassword = new Button(groupSftp, SWT.RADIO);
			butAuthPassword.addSelectionListener(new SelectionAdapter() {

				public void widgetSelected(final SelectionEvent e) {
					setEnabled();
					txtSFtpPassphrase.setEnabled(butAuthPassword.getSelection());
				}
			});
			...
			butAuthKeyboardInteractive = new Button(groupSftp, SWT.RADIO);
			butAuthKeyboardInteractive.addSelectionListener(new SelectionAdapter() {

				public void widgetSelected(final SelectionEvent e) {
					setEnabled();
					txtSFtpPassphrase.setEnabled(false);
				}
			});
			final GridData gridData_ki = new GridData(GridData.BEGINNING, GridData.END, false, false, 2, 1);
			...
			passphraseLabel
					.setText("Passphrase/Password. If left empty then the user will be prompted for keyboard input");
		...
		try {
			...
			if ("SFTP".equalsIgnoreCase(protocol)) {
				...
				if ("publickey".equalsIgnoreCase(method)) {
					...
					butAuthPublicKey.setSelection(false);
					butAuthKeyboardInteractive.setSelection(false);
				} else if ("keyboard-interactive".equalsIgnoreCase(method)) {
					butAuthKeyboardInteractive.setSelection(true);
					butAuthPublicKey.setSelection(false);
					butAuthPassword.setSelection(false);
				}
			} else {
	...
	private void apply() throws Exception {
		try {
			...
			throw new Exception("error in " + sos.util.SOSClassUtil.getMethodName()
	...
	private String getAuthMethod() {
		String authMethod = "";
		if (butAuthPublicKey.getSelection()) {
			authMethod = "publickey";
		} else if (butAuthPassword.getSelection()) {
			authMethod = "password";

		} else if (butAuthKeyboardInteractive.getSelection()) {
	...
	private void close() {
		...
		try {
			...
			if (butApply.getEnabled()) {
				int cont = message("Do you want to apply the changes you made?",
